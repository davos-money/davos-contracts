...
library AuctionProxy {
  ...
  function startAuction(address user, address keeper, IERC20Upgradeable davos, DavosJoinLike davosJoin, VatLike vat, DogLike dog, address davosProvider, CollateralType calldata collateral) public returns (uint256 id) {
    ...
    if (address(davosProvider) != address(0)) {
      IDavosProvider(davosProvider).daoBurn(user, _clip.sales(id).lot);
    }
  }

  function resetAuction(uint auctionId, address keeper, IERC20Upgradeable davos, DavosJoinLike davosJoin, VatLike vat, CollateralType calldata collateral) public {
  ...
  function buyFromAuction(uint256 auctionId, uint256 collateralAmount, uint256 maxPrice, address receiverAddress, IERC20Upgradeable davos, DavosJoinLike davosJoin, VatLike vat, address davosProvider, CollateralType calldata collateral) public returns (uint256 leftover) {
    ...
    if (address(davosProvider) != address(0)) {
      ...
      IDavosProvider(davosProvider).liquidation(receiverAddress, gemBal); // Burn router ceToken and mint amaticc to receiver
      ...
      if (leftover != 0) {
        ...
        IDavosProvider(davosProvider).liquidation(urn, leftover); // Router burns them and gives amaticc remaining
      }
    } else {
