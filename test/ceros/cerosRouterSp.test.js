...
describe('Ceros Router', () => {
    ...
    describe("Setter zero address reverts", async () => {
        it("reverts when 0 address passed to a setter", async () => {
            await expect(ce_rot.changePriceGetter(ZERO_ADDRESS)).to.be.revertedWith("");
            await expect(ce_rot.changeProvider(ZERO_ADDRESS)).to.be.revertedWith("");
            await expect(ce_rot.changeSwapPool(ZERO_ADDRESS)).to.be.revertedWith("");
            await expect(ce_rot.changeDex(ZERO_ADDRESS)).to.be.revertedWith("");
            await expect(ce_rot.changeVault(ZERO_ADDRESS)).to.be.revertedWith("");
        });
    });
});
...
async function init() {
    ...
    ce_token = await upgrades.deployProxy(CeToken, ["Ceros token", "ceAmaticc"], {initializer: "initialize"});
    ...
    ce_vault = await upgrades.deployProxy(ceVault, ["CeVault", ce_token.address, aMaticc.address], {initializer: "initialize"});
    // set vault for ceAMATICc
    await ce_token.changeVault(ce_vault.address);
    /* CeRot */
    const CeRot = await ethers.getContractFactory("CerosRouterSp");
