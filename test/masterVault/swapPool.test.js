  ...
  describe("# add/remove(userType)", () => {
    describe("reverts:", () => {
      it("if want to add zero address", async () => {
        ...
        await expect(lp.setSwapPool(swapPool.address)).to.be.revertedWith("swap pool can be set only once");
        await expect(lp.mint(swapPool.address, "1")).to.be.revertedWith("only swap pool can call this function");
      });
