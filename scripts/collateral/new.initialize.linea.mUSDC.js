...
async function main() {
    ...
    let { _oracle1 } = require(`../addresses_${hre.network.name}_oracle.json`);

    let _dog_chop = "1020000000000000000";
    let _mat = "1075268817204301100000000000";
    let _ilk = "0x4d56545f6d655553444300000000000000000000000000000000000000000000";

    let { _masterVault, _davosProvider, _dMatic, _clip, _gemJoin } = require(`../addresses_${hre.network.name}_collateral_mUSDC.json`);
    ...
    await spotAt["file(bytes32,bytes32,address)"](_ilk, ethers.utils.formatBytes32String("pip"), _oracle1, {nonce: _nonce}); _nonce += 1;
