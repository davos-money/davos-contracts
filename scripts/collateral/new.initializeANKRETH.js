...
async function main() {
    ...
    let { _masterVaultA, _davosProvider, _dMatic, _clip, _gemJoin } = require(`./addressesANKRETH_${hre.network.name}.json`);
    let _ilk = "0x4d56545f616e6b72455448000000000000000000000000000000000000000000";
    ...
    let masterVaultAt = await ethers.getContractAt("MasterVault_V2", _masterVaultA);
    ...
    // await oracleAt.initialize(aggregatorAddress, _underlying, _masterVaultA, {nonce: _nonce});
    ...
    await interactionAttached.setDavosProvider(_masterVaultA, _davosProvider, {nonce: _nonce}); _nonce += 1; console.log("1")
    await interactionAttached.setCollateralType(_masterVaultA, _gemJoin, _ilk, _clip, _mat, {nonce: _nonce}); _nonce += 1; console.log("2")
    // await interactionAttached.poke(_masterVaultA, {nonce: _nonce, gasLimit: 300000}); _nonce += 1; console.log("3")
    await interactionAttached.drip(_masterVaultA, {nonce: _nonce, gasLimit: 200000}); _nonce += 1; console.log("4")
    await interactionAttached.setCollateralDuty(_masterVaultA, _jug_duty, {nonce: _nonce, gasLimit: 250000}); _nonce += 1; console.log("5")
}
