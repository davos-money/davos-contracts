...
async function main() {
    ...
    let { _dog_hole, _dog_chop, _clip_buf, _clip_tail, _clip_cusp, _clip_chip, _clip_tip, _clip_stopped, _vat_line, _vat_dust, _jug_duty, _mat} = require(`./config_${hre.network.name}.json`);
    let { _ratioAdapter } = require(`./addresses_${hre.network.name}.json`);
    let {_oracle } = require(`../addresses_${hre.network.name}_oracle.json`);

    let _ilk = "0x4d56545f657a4554480000000000000000000000000000000000000000000000";

    let { _masterVault, _davosProvider, _dMatic, _clip, _gemJoin } = require(`../addresses_${hre.network.name}_collateral.json`);
