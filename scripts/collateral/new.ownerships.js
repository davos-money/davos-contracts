...
async function main() {
    ...
    await masterVaultAt.transferOwnership(_multisig); console.log("1");
    await davosProviderAt.transferOwnership(_multisig); console.log("2");
    await dMaticAt.transferOwnership(_multisig); console.log("3");
    await gemJoinAt.rely(_multisig); console.log("4");
    await clipAt.rely(_multisig);

    console.log("=== Try proxyAdmin transfer...");
    const proxyAdminAddress = parseAddress(await ethers.provider.getStorageAt(masterVaultAt.address, admin_slot));
