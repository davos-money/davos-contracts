...
async function main() {
    ...
    if (hre.network.name == "ethereum") {
        ...
        // _logic / admin_ / _data
        masterVault = await this.TUP.deploy("0xD6df23227f730aaB4C8eFa17268B694de75240Ca", "0x594399b8d67FEfFBF842A8833916971908b2D5c4", "0x", {nonce: _nonce}); _nonce += 1; await masterVault.deployed(); console.log("1");
        dMatic = await this.TUP.deploy("0x110C7dc503Abe9E5771A2FC0cAd77b26203be3FA", "0x594399b8d67FEfFBF842A8833916971908b2D5c4", "0x", {nonce: _nonce}); _nonce += 1; await dMatic.deployed(); console.log("2");
        davosProvider = await this.TUP.deploy("0x425CeC952837478C3ccb6e0aff92645E6d51cD7a", "0x594399b8d67FEfFBF842A8833916971908b2D5c4", "0x", {nonce: _nonce}); _nonce += 1; await davosProvider.deployed(); console.log("3");
        gemJoin = await this.TUP.deploy("0x0168e703acd8fE52B2Be3d908CfA92400e9A5fD8", "0x594399b8d67FEfFBF842A8833916971908b2D5c4", "0x", {nonce: _nonce}); _nonce += 1; await gemJoin.deployed(); console.log("4");
        clip = await this.TUP.deploy("0x86BFDE6F6E34Fd0F0Bd75e371909002053028B80", "0x594399b8d67FEfFBF842A8833916971908b2D5c4", "0x", {nonce: _nonce}); _nonce += 1; await clip.deployed(); console.log("5");
    ...
    fs.writeFileSync(`./scripts/addresses_${network.name}_collateral_lbtc.json`, json_addresses);
    console.log("Addresses Recorded to: " + `./scripts/addresses_${network.name}_collateral_lbtc.json`);
