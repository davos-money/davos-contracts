...
async function main() {
    ...
    let { _oracleUSDC } = require(`../addresses_${hre.network.name}_oracle_ion.json`);

    let _ilk = "0x4d56545f696f6e55534443000000000000000000000000000000000000000000";

    let { _masterVault, _davosProvider, _dMatic, _clip, _gemJoin } = require(`../addresses_${hre.network.name}_collateral_ionUSDC.json`);
    ...
    await spotAt["file(bytes32,bytes32,address)"](_ilk, ethers.utils.formatBytes32String("pip"), _oracleUSDC, {nonce: _nonce}); _nonce += 1;

    console.log("Gemjoin init...");
    await gemJoinAt.rely(_interaction, {nonce: _nonce}); _nonce += 1;

    console.log("RatioAdatper...");
    let ra = await ethers.getContractAt("RatioAdapter", "0x046B71694B3b659F491247167EDa42E0556123cf");
    await ra.setToken("0x2BE717340023C9e14C1Bb12cb3ecBcfd3c3fB038", "", "", "exchangeRateCurrent()", true, {nonce: _nonce}); _nonce += 1;
    await ra.setToken("0x94812F2eEa03A49869f95e1b5868C6f3206ee3D3", "", "", "exchangeRateCurrent()", true, {nonce: _nonce}); _nonce += 1;
