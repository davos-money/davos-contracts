...
async function main() {
    ...
    let { _dog_hole, _clip_buf, _clip_tail, _clip_cusp, _clip_chip, _clip_tip, _clip_stopped, _vat_line, _jug_duty, _dog_chop, _mat } = require(`./config_${hre.network.name}.json`);
    let { _ratioAdapter } = require(`./addresses_${hre.network.name}.json`);
    let { _oracle3 } = require(`../addresses_${hre.network.name}_oracle.json`);
    ...
    await spotAt["file(bytes32,bytes32,address)"](_ilk, ethers.utils.formatBytes32String("pip"), _oracle3, {nonce: _nonce}); _nonce += 1;
