...
async function main() {
    ...
    let { _masterVaultW, _davosProvider, _dMatic, _clip, _gemJoin } = require(`./addressesWSTETH_${hre.network.name}.json`);
    let _ilk = "0x4d56545f77737445544800000000000000000000000000000000000000000000";
    ...
    let masterVaultAt = await ethers.getContractAt("MasterVault_V2", _masterVaultW);
    ...
    // await oracleAt.initialize(aggregatorAddress, _underlying, _masterVaultW, {nonce: _nonce});
    ...
    await interactionAttached.setDavosProvider(_masterVaultW, _davosProvider, {nonce: _nonce}); _nonce += 1; console.log("1")
    await interactionAttached.setCollateralType(_masterVaultW, _gemJoin, _ilk, _clip, _mat, {nonce: _nonce}); _nonce += 1; console.log("2")
    // await interactionAttached.poke(_masterVaultW, {nonce: _nonce, gasLimit: 300000}); _nonce += 1; console.log("3")
    await interactionAttached.drip(_masterVaultW, {nonce: _nonce, gasLimit: 200000}); _nonce += 1; console.log("4")
    await interactionAttached.setCollateralDuty(_masterVaultW, _jug_duty, {nonce: _nonce, gasLimit: 250000}); _nonce += 1; console.log("5")
}
