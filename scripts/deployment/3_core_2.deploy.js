...
async function main() {
    ...
    let vow = await upgrades.deployProxy(this.Vow, [_vat, _davosJoin, _multisig], {initializer: "initialize", nonce: _nonce}); _nonce += 1;
    await vow.deployed();
    vowImp = await upgrades.erc1967.getImplementationAddress(vow.address);
    console.log("Vow             :", vow.address);
    console.log("VowImp          :", vowImp);

    let dog = await upgrades.deployProxy(this.Dog, [_vat], {initializer: "initialize", nonce: _nonce}); _nonce += 1;
    await dog.deployed();
    dogImpl = await upgrades.erc1967.getImplementationAddress(dog.address);
    console.log("Dog             :", dog.address);
    console.log("DogImp          :", dogImpl);

    let clip = await upgrades.deployProxy(this.Clip, [_vat, _spot, dog.address, _ilkCeMatic], {initializer: "initialize", nonce: _nonce}); _nonce += 1;
    await clip.deployed();
    ...
    fs.writeFileSync(`./scripts/deployment/addresses_${hre.network.name}_3_2.json`, json_addresses);
    console.log("Addresses Recorded to: " + `./scripts/deployment/addresses_${hre.network.name}_3_2.json`);
}
