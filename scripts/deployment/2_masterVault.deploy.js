...
async function main() {
    ...
    // Config
    // let { _wMatic, _aMATICc, _maxDepositFee, _maxWithdrawalFee, _maxStrategies, _waitingPoolCap, _swapPool } = require(`./config_${hre.network.name}.json`);
    ...
    // this.CerosYieldConverterStrategy = await hre.ethers.getContractFactory("CerosYieldConverterStrategySp");
    this.CerosYieldConverterStrategy = await hre.ethers.getContractFactory("CerosYieldConverterStrategyLs");

    // Deployment
    console.log("MasterVault...");

    // let masterVault = await upgrades.deployProxy(this.MasterVault, [_wMatic, "CEROS MATIC Vault Token", "ceMATIC", _maxDepositFee, _maxWithdrawalFee, _maxStrategies], {initializer: "initialize", nonce: _nonce}); _nonce += 1;
    ...
    // let waitingPool = await upgrades.deployProxy(this.WaitingPool, [masterVault.address, _wMatic, _waitingPoolCap], {initializer: "initialize", nonce: _nonce}); _nonce += 1;
    ...
    // let cerosYieldConverterStrategy = await upgrades.deployProxy(this.CerosYieldConverterStrategy, [_cerosRouter, deployer.address, _wMatic, _aMATICc, masterVault.address, _swapPool], {initializer: "initialize", nonce: _nonce}); _nonce += 1;
