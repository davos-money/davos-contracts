...
async function main() {
  ...
  console.log("--vat");
  await (await _vat.rely(_multisig, {nonce: _nonce})).wait(); _nonce += 1;
  console.log("---vat" + _multisig);

  console.log("--spot");
  await (await _spot.rely(_multisig, {nonce: _nonce})).wait(); _nonce += 1;
  console.log("---spot" + _multisig);

  console.log("--davos");
  await (await _davos.rely(_multisig, {nonce: _nonce})).wait(); _nonce += 1;
  console.log("---davos" + _multisig);

  console.log("--gemjoin");
  await (await _gemjoin.rely(_multisig, {nonce: _nonce})).wait(); _nonce += 1;
  console.log("---gemjoin" + _multisig);

  console.log("--davosjoin");
  await (await _davosjoin.rely(_multisig, {nonce: _nonce})).wait(); _nonce += 1;
  console.log("---davosjoin" + _multisig);

  console.log("--jug");
  await (await _jug.rely(_multisig, {nonce: _nonce})).wait(); _nonce += 1;
  console.log("---jug" + _multisig);

  console.log("--vow");
  await (await _vow.rely(_multisig, {nonce: _nonce})).wait(); _nonce += 1;
  console.log("---vow" + _multisig);

  console.log("--dog");
  await (await _dog.rely(_multisig, {nonce: _nonce})).wait(); _nonce += 1;
  console.log("---dog" + _multisig);

  console.log("--clip");
  await (await _clip.rely(_multisig, {nonce: _nonce})).wait(); _nonce += 1;
  console.log("---clip" + _multisig);

  console.log("--abaci");
  await (await _abaci.rely(_multisig, {nonce: _nonce})).wait(); _nonce += 1;
  console.log("---abaci" + _multisig);

  console.log("--dgtrewards");
  await (await _dgtrewards.rely(_multisig, {nonce: _nonce})).wait(); _nonce += 1;
  console.log("---dgtrewards" + _multisig);

  console.log("--interaction");
  await (await _interaction.rely(_multisig, {nonce: _nonce})).wait(); _nonce += 1;
  console.log("---interaction" + _multisig);

  console.log("--jar");
  await (await _jar.rely(_multisig, {nonce: _nonce})).wait(); _nonce += 1;
